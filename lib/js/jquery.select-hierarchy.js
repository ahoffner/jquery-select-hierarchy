...
(function($){
    ...
    $.fn.selectHierarchy = function(options) {     
		this.each(function() {
			...
			obj.wrap('<span class="drilldown-wrapper chain-selects" />');
			obj.after('<select class="drilldown-1 form-control"><option value="">' + options.placeholder + '</option></select>');
			...
			var change_handler = function(){
				...
				if (this_select.val()) {
					...
					obj.val(this_select.prevAll('select').first().val());
				} else {
					obj.val('');
				}
				   
				this_select.nextAll().remove();
						   
				// Check to see if there's any children, if there are we build another select box;
				if (node && node.children.length > 0) {
					this_select.after('<span class="glyphicon glyphicon-chevron-right"></span><select><option value="">' + options.placeholder +'</option></select>');
			
					var next_select = this_select.nextAll('select').first();
					next_select.addClass('drilldown-' + (node.depth + 1));
					next_select.addClass('form-control');
			...
			var segments = selected_label != '' ? selected_label.split(options.separator) : [];

			var labels = [];
			for (var i in segments) {
				labels.push(segments.slice(0, parseInt(i)+1).join(options.separator));
			}
			...
			$.each(labels, function() {
			  $('select.drilldown-' + counter, obj.parent()).val(choices_by_label[(this)].value);
